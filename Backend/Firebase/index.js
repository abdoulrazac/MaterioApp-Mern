...
const serviceAccount = require('../serviceAccountKey.json')             // private key
...
router.post('/upload',upload.single('file'), async(req,res)=> {                     // upload image to firebase
    try {
        const fileName = req.file.originalname;

       await b.file(fileName).createWriteStream().end(req.file.buffer)
       res.status(200).send({status:0, message:'Successfully upload to firebase'})
